...
class SequentialList():
    def __init__(self):
        ...
        self.box_x, self.box_y = 200, 200      #definig as parameter the x and y of the firstbox
        self.id = "Sequential list"
        self.mid_w, self.mid_h = self.wm.DISPLAY_W / 2, self.wm.DISPLAY_H / 2
        self.wm.blit_screen()
        self.flag_input = None
        self.input_box1= InputBox(self.box_x-50, self.box_y)
        self.input_box2 = InputBox(self.box_x-50, self.box_y+75)
        self.input_box3 = InputBox(self.box_x*2+75, self.box_y)
        self.input_box4 = InputBox(self.box_x*2+75, self.box_y+75)
        self.input_box5 = InputBox(self.box_x*3+15, self.box_y)
        self.input_box6 = InputBox(self.box_x*3+15, self.box_y+75)
    ...
    def show_display(self):
        ...
        while running: 
            ...
            self.wm.window.blit(text_surface, (self.box_x-160, self.box_y+5))  #in the left of the firstbox
            
            text_surface = font.render("o elemento:", True, self.wm.PURPLE)
            self.wm.window.blit(text_surface, (self.box_x-160, self.box_y+75+5))  #in the left of the second box

            text_surface = font.render("INSERÇÃO", True, self.wm.PURPLE)       #in the top of first column
            self.wm.window.blit(text_surface, (self.box_x-50, self.box_y-30))  
            ...
            self.wm.window.blit(text_surface, (self.box_x*2-45, self.box_y+5))  #in the left of the firstbox
            text_surface = font.render("por elemento:", True, self.wm.PURPLE)
            self.wm.window.blit(text_surface, (self.box_x*2-45, self.box_y+75+5))  #in the left of the second box
            
            text_surface = font.render("REMOÇÃO", True, self.wm.PURPLE)
            self.wm.window.blit(text_surface, (self.box_x*2+75+5, self.box_y-30))  #in the top of second column
            self.input_box3.draw(self.wm.window)
            self.input_box4.draw(self.wm.window)
            
            text_surface = font.render("BUSCA", True, self.wm.PURPLE)
            self.wm.window.blit(text_surface, (self.box_x*3+30, self.box_y-30))  #in the top of third column
